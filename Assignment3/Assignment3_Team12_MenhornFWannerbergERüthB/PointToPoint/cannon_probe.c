...
int main (int argc, char **argv) {
	...
	for(cannon_block_cycle = 0; cannon_block_cycle < sqrt_size; cannon_block_cycle++){
		...
		MPI_Irecv(B_local_block_copy, B_local_block_size, MPI_DOUBLE,
		...
		for(C_index = 0, A_row = 0; A_row < A_local_block_rows; A_row++){
			...
			start = MPI_Wtime();
			if(!is_recv_row){
				MPI_Test(&recv_row_request, &is_recv_row, &status);
			}
			if(!is_recv_column){
				MPI_Test(&recv_column_request, &is_recv_column, &status);
			}
			mpi_time += MPI_Wtime() - start;
			start = MPI_Wtime();
		}
